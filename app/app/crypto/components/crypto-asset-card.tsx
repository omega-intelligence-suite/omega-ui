...
export function CryptoAssetCard({ asset, target }: AssetCardProps) {

  const [showRoiValue, setShowRoiValue] = useState(false);

  const changeValue = (asset.usdBalance * asset.change_24h) / 100;
  const changePercentage = asset.change_24h
  ...
  const changeValueToDisplay = showRoiValue ? changeValue : changePercentage;
  const changeValueToDisplayFormatted = showRoiValue ? formatROIValue(changeValue, "currency") : formatROIValue(changePercentage, "percentage");

  const getTradingViewUrl = (symbol: string) => {
    // Most crypto pairs on TradingView use USDT or USD
    // Format: BINANCE:BTCUSDT or COINBASE:BTCUSD
    const tradingSymbol = symbol.toUpperCase()

    // Popular stablecoins don't need a pair
    if (["USDT", "USDC", "BUSD", "DAI"].includes(tradingSymbol)) {
      return `https://www.tradingview.com/chart/?symbol=BINANCE:${tradingSymbol}USD`
    }

    // For other cryptos, use USDT pair on Binance
    return `https://www.tradingview.com/chart/?symbol=BINANCE:${tradingSymbol}USDT`
  }
          ...
          <div className="flex items-start justify-between">
            <div className="flex">
              ...
              <div className="flex flex-col justify-center">
                ...
                <span className="text-xs text-slate-500 font-mono">Units: {asset.balance > 0.009 ? asset.balance.toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 }) : asset.balance.toLocaleString("en-US", { minimumFractionDigits: 8, maximumFractionDigits: 8 })}</span>
              </div>
            </div>
            <div className="flex gap-4 items-center">
              <div className="flex gap-2 items-center">
                <div className="hover:text-slate-200 text-slate-500 transition-colors rounded-50 cursor-pointer">
                  <a href={`https://coinmarketcap.com/currencies/${asset.name.toLowerCase().replace(/\s+/g, "-")}/`} target="_blank" rel="noopener noreferrer"><SiCoinmarketcap /></a>
                </div>
                <div className="hover:text-slate-200 text-slate-500 transition-colors rounded-50 cursor-pointer">
                  <a href={getTradingViewUrl(asset.symbol)} target="_blank" rel="noopener noreferrer"><SiTradingview /></a>
                </div>
              </div>
              <p
                className="text-xs text-slate-400 font-mono cursor-pointer hover:underline"
                style={{ color: changeValueToDisplay > 0 ? colors.primary.green : colors.primary.red }}
                onClick={() => setShowRoiValue(!showRoiValue)}
                title="24h Change (click to toggle value)"
              >
                {changeValueToDisplayFormatted}
              </p>
            </div>
          </div>
              ...
              <div className="flex flex-col">
                ...
                <div className="flex gap-1 items-end">
                  <span className="text-sm text-slate-100 font-mono"
                    style={{
                      color: asset.average_price < asset.current_price ? colors.primary.green : colors.primary.red
                    }}>${asset.current_price > 0.009 ? asset.current_price?.toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 }) : asset.current_price?.toLocaleString("en-US", { minimumFractionDigits: 8, maximumFractionDigits: 8 })}</span>
